  ...
  if c1 == c2 then
    if isNothing l && isNothing lt then
      if isNothing t then
        ...
        1 + (fromJustInt t)
    else if isJust lt then
      fromJustInt lt
    else
      0
  else if isJust l && isJust lt && isJust t then
...
getRow rowChar topWord rowAbove row =
  let
    colChar =
      fromJustChar <| List.head topWord

    nextRowAbove =
      List.drop 1 rowAbove

    nextTopWord =
      List.drop 1 topWord

    l =
      List.head <| List.reverse row

    lt =
      List.head rowAbove

    t =
      List.head nextRowAbove

    nextValue =
      getCellValue rowChar colChar l lt t
  in
    if List.isEmpty rowAbove && List.isEmpty row then
      -- this makes no sense
      []
    else if List.isEmpty nextRowAbove then
      -- row 1 is empty so we've built our row
      row
    else if List.isEmpty row then
      -- The case of the first column
      getRow rowChar topWord rowAbove [ 1 + (fromJustInt lt) ]
    else
      -- row 1 still has stuff so keep going
...
buildMatrix matrix topWord rowChars prevRow =
  let
    currentChar =
      fromJustChar <| List.head rowChars

    nextRowChars =
      List.drop 1 rowChars

    newRow =
      getRow currentChar topWord prevRow []
  in
    case rowChars of
      [] ->
        matrix
      ...
      _ ->
        buildMatrix (List.append matrix [ newRow ]) topWord nextRowChars newRow
...
getMatrix topWord leftWord =
  let
    topLength =
      String.length topWord

    leftLength =
      String.length leftWord

    topList =
      String.toList topWord

    leftList =
      String.toList leftWord

    topStringList =
      stringToStringList topWord

    leftStringList =
      stringToStringList leftWord

    row1 =
      List.append [ " ", " " ] topStringList

    col1 =
      List.map (\ltr -> [ ltr ]) (stringToStringList leftWord)
    ...
    innerMatrix =
      buildMatrix initialInnerMatrix topList leftList row2

    innerStringMatrix =
      List.map (\row -> List.map (\ele -> toString ele) row) innerMatrix

    -- Build display matrix around inner matrix (mainly string conversions)
    row2AsStrings =
      [ " " ] ++ (fromJust <| List.head innerStringMatrix)

    restRows =
      List.map2 (List.append) col1 (List.drop 1 innerStringMatrix)
  in
